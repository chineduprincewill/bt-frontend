...
export const CompleteProfileButton = ({ stageToStartFrom, header }: { header?: boolean; stageToStartFrom?: UpdateProfileFlowStages }) => {
    ...
    return (
        <>
            {header ? (
                <div>
                    <button
                        className="edit-button flex-row centralize-y centralize-x !mr-12 md:!mr-0"
                        onClick={onClick}
                        style={{
                            ...style,
                            cursor: 'pointer',
                    ...
                    >
                        <p id="edit-btn">{text ?? 'Edit Profile'}</p>
                        {!noIcon && <img src={EditProfile} alt="" />}
                    </button>

                    <div className="flex-row centralize-y !hidden md:block" style={{ paddingTop: 15 }}>
                        <p style={{ color: '#333333' }}>
                            <span style={{ fontWeight: 'bold', color: 'black' }}>
                                {/* @ts-ignore */}
                                {user.profile?.profileStats?.followersCount ?? '0'}
                            </span>{' '}
                            connections
                        </p>
                        <img src={Location} height={15} width={15} style={{ marginLeft: 10, marginRight: 3 }} />

                        <p style={{ color: '#333333' }}>
                            <span style={{ fontWeight: 'bold', color: 'black' }}>
                                {/* @ts-ignore */}
                                {user.profile?.location?.state}, {user.profile?.location?.country}
                            </span>
                        </p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="flex-row " style={{ paddingTop: 15 }}>
                        <p style={{ color: '#333333' }}>
                            <span style={{ fontWeight: 'bold', color: 'black' }}>
                                {/* @ts-ignore */}
                                {user.profile?.profileStats?.followersCount
                                    ? //@ts-ignore
                                      user.profile.profileStats.followersCount
                                    : 0}
                            </span>{' '}
...
export const ConnectToProfileButton = ({ username }: { username?: string }) => {
    const { loggedUser } = useSelector((state: RootState) => state.auth);
